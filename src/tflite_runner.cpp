...
bool TFLiteRunner::AllocateTensors() {
    ...
    LOGI("Tensors allocated successfully");
...
bool TFLiteRunner::RunInference(const std::vector<float>& input_data, 
    ...
    if (input_tensor->type == kTfLiteFloat32) {
        ...
        for (size_t i = 0; i < input_data.size(); i++) {
            input_ptr[i] = static_cast<uint8_t>(input_data[i]);
        }
    } else if (input_tensor->type == kTfLiteInt8) {
        int8_t* input_ptr = interpreter_->typed_input_tensor<int8_t>(0);
        for (size_t i = 0; i < input_data.size(); i++) {
            input_ptr[i] = static_cast<int8_t>(input_data[i]);
        }
    } else {
        LOGE("Unsupported input tensor type: %d", input_tensor->type);
        return false;
    ...
    // Copy output data
    if (output_tensor->type == kTfLiteFloat32) {
        const float* output_ptr = interpreter_->typed_output_tensor<float>(0);
        std::memcpy(output_data.data(), output_ptr, output_size * sizeof(float));
    } else if (output_tensor->type == kTfLiteUInt8) {
        const uint8_t* output_ptr = interpreter_->typed_output_tensor<uint8_t>(0);
        for (size_t i = 0; i < output_size; i++) {
            output_data[i] = static_cast<float>(output_ptr[i]);
        }
    } else if (output_tensor->type == kTfLiteInt8) {
        const int8_t* output_ptr = interpreter_->typed_output_tensor<int8_t>(0);
        for (size_t i = 0; i < output_size; i++) {
            output_data[i] = static_cast<float>(output_ptr[i]);
        }
    } else {
...
std::vector<int> TFLiteRunner::GetInputShape() const {
    std::vector<int> shape;
    if (interpreter_ && interpreter_->inputs().size() > 0) {
        TfLiteTensor* tensor = interpreter_->input_tensor(0);
        for (int i = 0; i < tensor->dims->size; i++) {
            shape.push_back(tensor->dims->data[i]);
        }
    }
...
std::vector<int> TFLiteRunner::GetOutputShape() const {
    std::vector<int> shape;
    if (interpreter_ && interpreter_->outputs().size() > 0) {
        TfLiteTensor* tensor = interpreter_->output_tensor(0);
        for (int i = 0; i < tensor->dims->size; i++) {
            shape.push_back(tensor->dims->data[i]);
        }
    }
